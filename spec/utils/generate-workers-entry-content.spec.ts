...
describe('generate-workers-entry-content', () => {
  it('matches snapshot for single worker', () => {
    const content = generateWorkersEntryContent(['/path/to/worker.mjs'])
    expect(content).toMatchSnapshot()
  })

  it('matches snapshot for multiple workers and redis url', () => {
    const content = generateWorkersEntryContent([
      '/app/server/workers/basic.ts',
      '/app/server/workers/hello.ts',
    ])
    expect(content).toMatchSnapshot()
  })
  ...
  it('generates entry that parses --workers flag from process.argv', () => {
    ...
    expect(content).toContain(
      "process.argv.find(a => typeof a === 'string' && a.startsWith('--workers='))",
    )
    expect(content).toContain(
  ...
  it('generates entry that filters workers by name when --workers is set', () => {
    ...
    expect(content).toContain(
      'api.workers.filter(w => w && selectedWorkers.includes(w.name))',
    )
  })
  ...
  it('generates entry that waits for workers and rethrows startup failures', () => {
    const content = generateWorkersEntryContent(['/path/to/worker.mjs'])

    expect(content).toContain('await Promise.all(workersToRun.map((w) => w.waitUntilReady()))')
