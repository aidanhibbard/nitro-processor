...
describe('generate-workers-index-wrapper', () => {
  ...
  it('imports createWorkersApp from the given entry path', () => {
    ...
    expect(content).toContain(
      "import { createWorkersApp } from './chunks/_entry.mjs'",
    )
  })
  ...
  it('delegates shutdown timeout handling to stopAll', () => {
    const content = generateWorkersIndexWrapper('./_entry.mjs')

    expect(content).toContain(
      'await app.stop({ timeoutMs: SHUTDOWN_TIMEOUT_MS })',
    )
    expect(content).not.toContain('Promise.race')
    expect(content).not.toContain('app.stop({ force: true })')
  })
  ...
  it('exits with code 1 on startup failure', () => {
    ...
    expect(content).toContain(
      "createWorkersApp().catch((err) => { logger.error('failed to start workers', err); process.exit(1) })",
    )
  })
