...
describe('normalizeRedisConnectionEntry', () => {
  ...
  it('normalizes lazyConnect from booleans and strings', () => {
    ...
    expect(normalizeRedisConnectionEntry('lazyConnect', 'false')).toBe(false)
    expect(
      normalizeRedisConnectionEntry('lazyConnect', 'maybe'),
    ).toBeUndefined()
  })
  ...
  it('coerces port, db, and connectTimeout to numbers', () => {
    ...
    expect(normalizeRedisConnectionEntry('connectTimeout', '12000')).toBe(
      12_000,
    )
    expect(
      normalizeRedisConnectionEntry('port', 'not-a-number'),
    ).toBeUndefined()
    expect(normalizeRedisConnectionEntry('port', Number.NaN)).toBeUndefined()
  })

  it('passes through other keys unchanged', () => {
    expect(normalizeRedisConnectionEntry('host', 'redis.internal')).toBe(
      'redis.internal',
    )
