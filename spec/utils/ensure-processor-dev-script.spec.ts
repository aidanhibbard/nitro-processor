...
describe('ensureProcessorDevScript', () => {
  ...
  it('returns false when package.json does not exist', async () => {
    const result = await ensureProcessorDevScript(tmpDir, {
      ask: async () => 'y',
    })
  ...
  it('returns true when processor:dev script already exists', async () => {
    const pkg = {
      name: 'app',
      version: '0.0.0',
      scripts: { 'processor:dev': 'nitro-processor dev' },
    }
  ...
  it('returns false when package.json write fails', async () => {
    writeFileSync(
      join(tmpDir, 'package.json'),
  ...
  it('preserves existing scripts when adding processor:dev', async () => {
    const pkg = {
      name: 'app',
      version: '0.0.0',
      scripts: { build: 'nitro build', dev: 'nitro dev' },
    }
    writeFileSync(join(tmpDir, 'package.json'), JSON.stringify(pkg, null, 2))
    ...
    expect(updated.scripts?.build).toBe('nitro build')
    expect(updated.scripts?.dev).toBe('nitro dev')
    expect(updated.scripts?.['processor:dev']).toBe('nitro-processor dev')
  })
})
