...
vi.mock('nitro', () => ({
  definePlugin: (
    plugin: (nitroApp: { hooks: { hook: ReturnType<typeof vi.fn> } }) => void,
  ) => plugin,
}))
...
const importCloseProcessor = () =>
  import('../../../src/runtime/server/plugins/close-processor')
...
describe('close-processor plugin', () => {
  ...
  it('calls useProcessor().stopAll() when close hook runs', async () => {
    ...
    const closeHandler = nitroApp.hooks.hook.mock.calls.find(
      ([name]) => name === 'close',
    )?.[1]
