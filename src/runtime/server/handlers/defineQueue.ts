...
interface DefineQueueArgs<DefaultNameType extends string = string> {
...
>({
  name,
  options,
}: DefineQueueArgs<DefaultNameType>): Queue<
  DataTypeOrJob,
  DefaultResultType,
  DefaultNameType
> => {
  const { createQueue } = useProcessor()
  return createQueue<DataTypeOrJob, DefaultResultType, DefaultNameType>(
    name,
    options,
  )
}
