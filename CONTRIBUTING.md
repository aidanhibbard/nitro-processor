# Contributing to nitro-processor
...
- **Bug reports** — open an issue with reproduction steps, Nitro version, and Redis setup.
...
```bash
git clone https://github.com/aidanhibbard/nitro-processor.git
cd nitro-processor
npm install
npm run dev:prepare
```
...
npm run test          # Vitest (100% coverage enforced)
npm run ci            # lint + typecheck + test:types + test
```
...
Use clear, imperative subject lines (e.g. `fix: resolve Redis port from NITRO_*`).
...
- Beta: `npm publish --tag beta` — install with `nitro-processor@beta`.

## Questions

Open a [GitHub Discussion](https://github.com/aidanhibbard/nitro-processor/discussions)
